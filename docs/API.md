...
Der serverseitige `x-auth-token` wird nie an Browser ausgegeben.
...
DJI-`drone_model_key` und `payload_model_keys` werden als eigene
Produktidentitäten erhalten und nicht in MQTT-`payload_index` umgedeutet.
