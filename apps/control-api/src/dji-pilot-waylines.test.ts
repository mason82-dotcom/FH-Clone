...
import {
  ...
  withPilotWaylineMissionReferences
...
test("adds authoritative mission references to Pilot Wayline items", () => {
  const enriched = withPilotWaylineMissionReferences({
    items: [
      {
        id: "pilot-wayline-123",
        name: "Inspection",
        payloadModelKeys: [],
        templateTypes: [0]
      },
  ...
  assert.deepEqual(enriched.items[0]?.missionReference, {
    kind: "wayline",
    id: "pilot-wayline-123",
    source: "dji_pilot_wayline",
    confidence: "authoritative"
  });
  assert.equal(enriched.items[1]?.missionReference, null);
});
